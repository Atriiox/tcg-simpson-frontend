  ...
  const formik = useFormik<FriendFormValues>({
    ...
    onSubmit: async (values, { setFieldError, setSubmitting, resetForm }) => {
      if (!values.pseudo.trim()) {
        setFieldError("pseudo", "Le pseudo ne peut pas être vide");
        setSubmitting(false);
        return;
      }
      try {
    ...
    <div className="w-full flex-1 p-6 md:p-10 font-main text-simpson-dark dark:text-simpson-white select-none overflow-y-auto">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-simpson-gray/10 dark:border-white/10 pb-6">
          <div>
            <h1 className="text-2xl font-black uppercase tracking-wider text-simpson-orange dark:text-simpson-yellow">
              Hub Social
            </h1>
            <p className="text-xs font-medium text-simpson-gray uppercase tracking-widest mt-1">
              Gère ta liste d'amis et prépare tes futurs duels
            </p>
          </div>

          <div ref={autocompleteRef} className="relative w-full md:w-auto">
            <form onSubmit={formik.handleSubmit} className="flex gap-2 w-full md:w-auto">
              <input
                id="pseudo"
                name="pseudo"
                type="text"
            ...
            <div className="h-5 mt-1 px-1">
              {formik.errors.pseudo && <p className="text-red-500 text-[11px] font-bold uppercase truncate">{formik.errors.pseudo}</p>}
            </div>

            {suggestions.length > 0 && (
              <div className="absolute left-0 right-0 top-[46px] bg-white dark:bg-simpson-darklight border border-simpson-gray/20 dark:border-white/10 rounded-xl shadow-lg z-50 overflow-hidden">
                {suggestions.map((suggestion) => (
                  <button
                    key={suggestion.pseudo}
                    type="button"
                    onClick={() => {
                      setSuggestions([]);
                      setNewFriendPseudo("");
            ...
            {friends.map((friend, i) => (
              <div key={i} className="bg-white dark:bg-simpson-darklight border border-simpson-gray/5 rounded-2xl p-5 shadow-xs relative">
                ...
                <div className="flex items-center gap-4 mb-5 mt-2">
                  <div className="w-14 h-14 rounded-full bg-simpson-gray/10 relative border-2 border-simpson-orange/20 dark:border-simpson-yellow/20 overflow-hidden">
