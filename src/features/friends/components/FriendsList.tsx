...
export default function FriendsList() {
  const {
    ...
    setSuggestions,
    handleAddFriend,
    handleRemoveFriend, // 🎯 Récupération de la fonction de suppression
    loadFriends,
  } = useFriends();
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-simpson-gray/10 dark:border-white/10 pb-6">
          ...
          <div ref={autocompleteRef} className="relative w-full md:w-auto">
            ...
            <div className="h-5 mt-1 px-1">
              {formik.errors.pseudo && (
                <p className="text-red-500 text-[11px] font-bold uppercase truncate">
                  {formik.errors.pseudo}
                </p>
              )}
            </div>
        ...
        {friends.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {friends.map((friend, i) => (
              ...
              >
                {/* 🎯 Bouton Poubelle branché sur le pseudo unique */}
                <button
                  onClick={() => handleRemoveFriend(friend.pseudo)}
